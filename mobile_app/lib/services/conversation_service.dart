...
/// Service for managing chat conversations with Firebase optimization
class ConversationService extends ChangeNotifier {
  ...
  Future<List<ChatConversation>> getUserConversations(String userId) async {
    ...
    try {
      // Fetch from Firestore with optimization
      ...
      await _usageMonitor.recordOperation('read', query.docs.length);
  ...
  Future<ChatConversation> createConversation({
    ...
    try {
      ...
      if (_conversationCache[userId]!.length > ChatConfig.maxConversationsPerUser) {
      ...
      // Track Firebase usage
      await _usageMonitor.recordOperation('write', 1);
  ...
  Future<List<ChatMessage>> getConversationMessages({
    ...
    try {
      ...
      // Track Firebase usage
      await _usageMonitor.recordOperation('read', querySnapshot.docs.length);
  ...
  Future<List<ChatConversation>> searchConversations({
    ...
    try {
      ...
      if (_conversationCache.containsKey(userId)) {
        final cached = _conversationCache[userId]!;
        final filtered = cached.where((conversation) =>
      ...
      // If not found in cache, search in Firestore
  ...
  Future<void> _cacheConversationLocally(ChatConversation conversation) async {
    try {
      ...
      await prefs.setString(key, conversation.toFirestore().toString());
    } catch (e) {
  ...
  Future<List<ChatConversation>?> _getLocalConversations(String userId) async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final keys = prefs.getKeys().where((key) => key.startsWith('conversation_$userId'));
  ...
  Map<String, int> getCacheStats() {
    return {
      'conversations': _conversationCache.values.fold(0, (sum, list) => sum + list.length),
      'messages': _messageCache.values.fold(0, (sum, list) => sum + list.length),
