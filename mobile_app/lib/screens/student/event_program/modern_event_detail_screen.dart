...
class _ModernEventDetailScreenState extends State<ModernEventDetailScreen>
  ...
  EventRegistrationModel? _registrationDetails;
  ...
  Widget _buildRegistrationStatusBanner() {
    ...
    if (widget.event.registrationOpen == null) {
      ...
      return const SizedBox.shrink(); // Don't show banner if no registration fields
    }
  ...
  Widget _buildRegisterButton() {
    ...
    final bool canRegister = !hasRegistrationFields || (widget.event.canRegister && !_isRegistered);
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        color: !canRegister ? AppTheme.textSecondaryColor.withValues(alpha: 0.3) : null,
        borderRadius: BorderRadius.circular(AppTheme.radiusMd),
  ...
  Future<void> _loadRegistrationStatus() async {
    ...
    try {
      ...
      if (mounted) {
        ...
        // Load registration details if registered
        if (isRegistered) {
          final details = await _eventService.getRegistrationDetails(
  ...
  Future<void> _registerForEventInApp() async {
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            ...
            const SizedBox(height: AppTheme.spaceMd),
            const Text('Your profile information will be automatically filled:'),
            const SizedBox(height: AppTheme.spaceSm),
            ...
            _buildInfoRow('Email',
                SupabaseConfig.auth.currentUser?.email ?? 'N/A'),
            if (widget.event.eventDate != null) ...[
              const SizedBox(height: AppTheme.spaceSm),
              const Divider(),
              const SizedBox(height: AppTheme.spaceSm),
              _buildInfoRow('Event Date',
                  DateFormat('dd MMM yyyy, hh:mm a').format(widget.event.eventDate!)),
            ],
    ...
    try {
      print('========================================');
      print('🔵 UI: STARTING REGISTRATION PROCESS');
      print('Event ID: ${widget.event.id}');
      print('Event Title: ${widget.event.title}');
      print('User Profile: ${_userProfile?.fullName}');
      print('========================================');
      ...
      print('========================================');
      print('🔵 UI: REGISTRATION COMPLETED');
      print('Result: ${registration != null ? "SUCCESS ✅" : "FAILED ❌"}');
      print('========================================');
      debugPrint('🔵 UI: Registration completed, result: ${registration != null ? "Success" : "Failed"}');
      ...
      if (mounted) {
        ...
        if (registration != null) {
          ...
          if (isAlreadyRegistered) {
            ...
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
                content: Text('Registration failed. Please check your internet connection and try again.'),
                backgroundColor: AppTheme.errorColor,
  ...
  @Deprecated('Use _registerForEventInApp instead')
